...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		slug = self.sanitize_title(convert['name'])
		# {'name': 'Detoxification', 'slug': 'detoxification', 'term_group': 0}
		category_term = {
			# 'term_id' : convert['id'],
			'name': convert['name'],
			'slug': convert['code'] if convert['code'] else slug,
		...
		if self._notice['target']['config'].get('category'):
			taxonomy = self._notice['target']['config']['category']
		parent_id = self.get_map_field_by_src(map_type = self.TYPE_CATEGORY, id_src = convert['parent']['id'])
		category_taxonomy = {
		...
		if product_id:
			...
			try:
				meta_data = {
					...
					'meta_value': php_serialize(all_attr)
				}
				additional_meta_product_query = self.create_insert_query_connector('postmeta', meta_data)
				test = self.import_data_connector(additional_meta_product_query)
			except:
				pass
	...
	def product_import(self, convert, product, products_ext):
		...
		product_posts_data = {
			...
			'post_modified_gmt': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
				'created_at'] else get_current_time(),
			# 'post_parent': convert['relate']['parent'],
			'post_type': 'product',
			'comment_count': 0,
			'guid': '',
			'post_excerpt': '',
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
			...
			image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(
				'wp-content/uploads/'+ self.make_woocommerce_image_path(image_process['path'], self.TYPE_PRODUCT),
				self._notice['target']['config']['image_product'].rstrip('/')), override = 'override')
			...
			if image_import_path:
				...
				image_details = self.get_sizes(image_process['url'])
				# print(image_details) #(121931, (2000, 2000))
				thumbnail_id = self.wp_image(cate_image, image_details)
		...
		all_query = []
		for meta_key, value in product_metadata.items():
			postmeta_data = {
				'post_id': product_id,
				'meta_key': meta_key,
				'meta_value': value
			...
			all_query.append(self.create_insert_query_connector('postmeta', postmeta_data))
		self.import_multiple_data_connector(all_query, 'product', True)
		...
		if convert['categories']:
			...
			for cate_id in all_categories:
				category_data = {
					'object_id': product_id,
					'term_taxonomy_id': self.get_map_field_by_src(map_type = self.TYPE_CATEGORY, id_src = cate_id,
					                                              field = 'code_desc') if self.get_map_field_by_src(map_type = self.TYPE_CATEGORY, id_src = cate_id,
					                                                                                                field = 'code_desc') else '',
					'term_order': 0
				}
				category_query = self.create_insert_query_connector("term_relationships", category_data)
				all_queries.append(category_query)
	...
	def customer_import(self, convert, customer, customers_ext):
		users_metadata = {
			...
			'display_name': f"{convert['first_name']} {convert['last_name']}",
			'user_activation_key': ''
		}
		customer_queries = self.create_insert_query_connector('users', users_metadata)
		customer_id = self.import_data_connector(customer_queries, 'ID')
		...
		all_query = []
		for meta_key, value in users_meta.items():
			usermeta_data = {
				'user_id': customer_id,
				'meta_key': meta_key,
				'meta_value': value
			...
			all_query.append(self.create_insert_query_connector('usermeta', usermeta_data))
		self.import_multiple_data_connector(all_query, 'customer', True)
		...
		for address in convert['address']:
			if address['id'] == customer['customers_default_address_id']:
				customer_data_lookup['country'] = address['country']['country_code'] if address['country']['country_code'] else ''
				customer_data_lookup['postcode'] = address['postcode'] if address['postcode'] else ''
				customer_data_lookup['city'] = address['city'] if address['city'] else ""
				customer_data_lookup['state'] = address['state']['name'] if address['state']['name'] else ''
	...
	def order_import(self, convert, order, orders_ext):
		order_dataset = {
			...
			'post_date': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
				'created_at'] else get_current_time(),
			'post_date_gmt': convert['updated_at'] if convert['updated_at'] and '0000-00-00' not in convert[
				'updated_at'] else get_current_time(),
			'post_content': "",
			'post_title': 'Order &ndash;',
			'post_status': 'wc-completed' if convert['status'] == '1' else 'wc-processing',
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		oder_metadata = {
			# '_customer_user': convert['client_customers_id'] if convert['client_customers_id'] else 0,
			'_order_currency': convert['currency'],
			'_order_shipping_tax': convert['shipping']['amount'] if convert['shipping']['amount'] else 0,
			'_order_tax': convert['tax']['amount'] if convert['tax']['amount'] else 0,
			'_order_total': convert['total']['amount'] if convert['total']['amount'] else 0,
			...
			'_order_shipping': convert['shipping']['amount'] if convert['shipping']['amount'] else 0,
			'_order_key': '',
			'_payment_method': convert['payment']['method'],
			'_payment_method_title': convert['payment']['title'],
			'_created_via': '',
			'_date_completed': int(time.time()),
			'_date_paid': int(time.time()),
			'_billing_first_name': convert['customer']['first_name'],
			'_billing_last_name': convert['customer']['last_name'],
			'_billing_company': '',
			...
			'_billing_country': convert['billing_address']['country']['name'],
			'_billing_email': '',
			'_billing_phone': '',
			'_shipping_first_name': convert['customer']['first_name'],
			'_shipping_last_name': convert['customer']['last_name'],
			'_shipping_company': '',
			...
			'_shipping_country': convert['shipping_address']['country']['name'],
		...
		all_query = []
		for meta_key, value in oder_metadata.items():
			postmeta_data = {
				'post_id': order_id,
				'meta_key': meta_key,
				'meta_value': value
		...
		self.import_multiple_data_connector(all_query, 'order', True)
		...
		for item in convert['items']:
			...
			item_metadata = {
				...
				'_qty': item['qty'],
				'_tax_class': '',
				'_line_subtotal': float(item['price'] * int(item['qty'])),
				'_line_subtotal_tax': 0,
				'_line_total': item['total'],
				'_line_tax': item['tax_amount'],
				'_line_tax_data': 'a:2:{s:5:"total";a:1:{i:1;s:1:"0";}s:8:"subtotal";a:1:{i:1;s:1:"0";}}',
				'_reduced_stock': item['qty']

			}

			item_metadata_all_query = []
			for meta_key, value in item_metadata.items():
				item_meta_data = {
					'order_item_id': order_item_id,
					'meta_key': meta_key,
					'meta_value': value
				...
				item_metadata_all_query.append(self.create_insert_query_connector('woocommerce_order_itemmeta', item_meta_data))

			self.import_multiple_data_connector(item_metadata_all_query, 'item_metadata', True)
