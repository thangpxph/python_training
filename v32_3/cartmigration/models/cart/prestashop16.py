...
class LeCartPrestashop16(LeBasecart):
	...
	def get_customers_ext_export(self, customers):
		...
		customer_ext_queries = {
			...
			'customer_reward': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_loyalty WHERE id_customer IN " + self.list_to_in_condition(customer_ids)
			},
			'group': {
				'type': 'select',
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		rewards = get_list_from_list_by_field(customers_ext['data']['customer_reward'], 'id_customer', customer['id_customer'])
		if rewards:
			customer_data['rewards'] = rewards
